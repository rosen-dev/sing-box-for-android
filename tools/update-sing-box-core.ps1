...
Write-Host "            Sing-box Go 核心 (libbox.aar) 智能同步工具           " -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
...
Write-Host "[*] 正在查询官方 sing-box 最新正式版核心版本..." -ForegroundColor Yellow
$remoteTag = $null
try {
    ...
    Write-Host "[!] 无法连接 GitHub API 查询最新版本，将基于文件存在性进行判断。" -ForegroundColor Yellow
}
...
try {
    Write-Host "[*] 正在从 GitHub Releases 直链高速下载 libbox.aar (约 117MB)..." -ForegroundColor Cyan
    ...
    if ($remoteTag) {
        Write-Host ("    核心版本: " + $remoteTag) -ForegroundColor Green
    }
    Write-Host "================================================================" -ForegroundColor Cyan
