...
| 工具文件 | 作用场景 | 常用命令 |
| :--- | :--- | :--- |
| **`rebase-to-latest-clients-tag.ps1`** | **【客户端代码同步】** 官方 Android 仓库发新版时，自动将本项目的自定义功能变基至官方最新主线提交（自带智能备份与回滚机制，并自动联动核心检查） | `.\tools\rebase-to-latest-clients-tag.ps1` |
| **`update-sing-box-core.ps1`** | **【Go 核心库同步】** 官方 Go 核心发新版时，一键从 GitHub Releases 下载最新编译的 `libbox.aar` 覆盖到 `app/libs/`（版本未变秒级跳过，并自动联动配置规范诊断） | `.\tools\update-sing-box-core.ps1` |
| **`validate-gateway-config.ps1`** | **【配置与规则兼容性体检】** 一键深度诊断 4 大规则集语法、Sing-box 1.14+ 现代 DNS/Route 规范兼容性，确保核心升级后配置 100% 合规 | `.\tools\validate-gateway-config.ps1` |
